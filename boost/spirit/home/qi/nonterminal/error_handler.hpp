...
namespace boost { namespace spirit { namespace qi
{
    ...
    struct error_handler2 
    {
        ...
        error_handler2(function_type subject, F f)
          : subject(subject)
          , f(f)
        {
        }
    ...
    template <
        typename ID, error_handler_result action
      , typename Iterator, typename T0, typename T1, typename T2
      , typename F>
    void on_error(rule<Iterator, T0, T1, T2>& r, F f)
    {
    ...
    // Error handling support when <action> is not
    // specified. We will default to <fail>.
    template <typename ID, typename Iterator, typename T0, typename T1
      , typename T2, typename F>
    void on_error(rule<Iterator, T0, T1, T2>& r, F f)
    {
        on_error<ID, fail>(r, f);
    }    
}}}
